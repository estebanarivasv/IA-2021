...
class Floor:
	...
	def __init__(self, N):
		self.size = N
		self.tiles = [Tile(choices) for x in range(self.size)]
		# self.tiles = [
		# 	Tile('*'),
		# 	Tile('*'),
		# 	Tile('*'),
	...
	def ramdon_dirt(self):
		new_tile_position = random.randint(0, self.size - 1)
		if self.tiles[new_tile_position].state == '#':
			pass
		else:
			self.tiles[new_tile_position] = Tile(dirt)
